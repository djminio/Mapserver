...
void LoadPublicNotice( void )
{
	...
	sprintf_s(filename, sizeof(filename), ".\\data\\공지사항.txt");
	...
	p.h.header.type = CMD_GET_SERVER_TEXT ;
	{
		switch( type )
		{
			case CMD_PUBLIC_NOTICE_BBS_REQ:	{
												...
												{
													szTemp = new char[100];
													sprintf_s( szTemp, sizeof(szTemp), "내용이 없습니다." );
												}
...
void SendLoginServePreachNameOk( k_server_preach_name *p, short int cn )		// 이 함수는 전도한 사람에게 신앙수치를 올려주는 루틴이다.
{
	...
	strcpy_s(ch->preach_name, sizeof(ch->preach_name), p->name); // 전도자의 이름을 넣고
...
//
////////////////////////////////////////////////////////////////////////////
void CheckChatingData( char *name, char *message, unsigned char type )		// 채팅하는 말 저장 하기
{
	string strPath;
...
//	str_small = new char[total_len];		// 가상으로 바꾼값으로 비교해서 처리하게끔하기 위해
	...
	_strlwr_s( str_small, sizeof(str_small) );		// 소문자로 바꾼다.
...
void CheckAndSendChangeEquip( CHARLIST *ch, int send  )		// send가 1이면 무조건 보내준다.
{
	...
	if(HorseNo>=8041&&HorseNo<=8043)
	{
		if (ch->bAlive==1)
		{
			...
			ch->HorseGrade=GetHorseGrade(ch);//020508 LTS 용량 문제
			ch->RiderGrade=GetRiderGrade(ch);//020508 LTS 용량 문제
		/*	ch->HorseNo=2;
			ch->RiderNo=1;
			ch->HorseGrade=1;
			ch->RiderGrade=1;*/
...
void RecvBuyVisa( t_packet *p, short int cn )
{
	...
	static int of_nation[] = { N_VYSEUS, N_ZYPERN, N_YLLSE, N_VYSEUS, N_ZYPERN, N_YLLSE };		// 비자의 원래 국가
...
void RecvReporterMode( t_packet *p, short int cn )
{
	...
	if( !ch->IsReporter() && !ch->IsCounselor()) return;		// 기자가 아니다. //Eleval added GM in that so can check on normal client if needed
