...
static USER_FUNC function1[ MAX_SCR_ARRAY ];           // À¯Àú°¡ Á¤ÀÇÇÑ ÇÔ¼ö¸¦ ÀúÀå
...
int SearchFunction( char *s )
{
	...
	for( i = 0; function1[i].s[0] != 0; i++ )
        if( !strcmp( function1[i].s, s ) ) return( i );
...
int GetFunction( void )
{
    ...
    WriteBYTE( (char)function1[tok].n );          // ÇÔ¼öÀÇ ¹øÈ£¸¦ ¼ÂÆÃ

    len = strlen( function1[tok].format );  // ¸Å°³º¯¼öÀÇ ±æÀÌ¸¦ ¾òÀ½

    if( GetToken() == -1 ) return -1;
    if( *TOKEN != '(' ){ Serror( 1 );  return -1; }

    for( i = 0; i < len; i++ ) {
        switch( function1[tok].format[i] ) {
            case 'i':                      // intÇüÀ» Ã³¸®
                if( GetToken() == -1 ) return -1;
                switch( TOKENTYPE )
                {
...
int  LoadData( char *szdat )
{
	...
	while( 1 ) {
		...
		while( 1 ) {
			...
			strcpy( function1[i].s, TOKEN );

			GetToken ();
			if( TOKENTYPE != STRING ) { Serror( 1 ); return -1; }
			if( !strcmp( TOKEN, "null" ) )              // ¸Å°³º¯¼ö°¡ ¾øÀ¸¸é
				function1[i].format[0] = 0;
			else
				strcpy( function1[i].format, TOKEN );    // ¸Å°³º¯¼ö°¡ ÀÖÀ¸¸é
...
{	
	...
	memset( function1, 0, sizeof( USER_FUNC ) * 1000 ); //MAX_SCR_ARRAY );           // À¯Àú°¡ Á¤ÀÇÇÑ ÇÔ¼ö¸¦ ÀúÀå
