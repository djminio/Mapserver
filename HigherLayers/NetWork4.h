...
//<------------------------------- 1.04 addition ------------------------------------------
#define CMD_CHECK_WARFIELD_SQUAD_MEMBERCOUNT  26501	// 부대 인원 수를 테크한다. 패킷
#define  CMD_MONSTER_RAID_START  26502	// 몬스터 마을 습격을 시작한다 신규전쟁터맵서버->해당 맵서버
#define  CMD_MONSTER_RAID_END  26503	// 몬스터 마을 습격을 끝낸다. 신규전쟁터 맵서버-> 해당 맵서버.
#define  CMD_GUARD_CAN_NOT_BROKE  26504	// 성문이나 수호석을 부술 수 없다
#define CMD_CHECK_REMAIN_GUARD  26505	// 남은 성문이나 수호석을 반환한다.
...
//<------------------------------- 1.04+ addition ------------------------------------------
#define CMD_NW_MAP_PEACE_MOVE  26506;	// 평화 기간의 전쟁터 이동 요청 C -> M
#define CMD_NW_MAP_PEACE_MOVE_RESULT  26507;	// 평화 기간에 요청한 전쟁터 이동 결과 M -> C
#define CMD_WARFIELD_INFO = 26508;	// 전쟁터 소유자와 전쟁터 상태 요청 M -> WM
#define CMD_WARFIELD_INFO_RESULT  26509;	// 전쟁터 소유자와 전쟁터 상태 요청 WM -> M
...
struct NEWWAR_START
{
	DWORD			dwTerm;			// 몇분동안 열리는지
	char			WarfieldNo;
	unsigned char	LevelMin;
	unsigned char	LevelMax;
	unsigned char	btNationPoint;	// 국지전 국가 포인트
};
