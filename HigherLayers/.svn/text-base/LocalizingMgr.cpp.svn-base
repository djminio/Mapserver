...
void CLocalizingMgr::SetNationName(const char* szNationName)
{
	...
	strcpy_s(m_pszNationName, strlen(szNationName)+1,szNationName);
}
	...
	case TOTAL_DB:
		{
			...
			strcpy_s(m_pszTotalDbID, strlen(szId)+1,szId);
			strcpy_s(m_pszTotalDbPW, strlen(szPw)+1,szPw);
		}break;
	case DRAGON_DB:
		{
			...
			int size;
			size = sizeof(m_pszDragonDbID);
			strcpy_s(m_pszDragonDbID, strlen(szId)+1,szId);
			strcpy_s(m_pszDragonDbPW, strlen(szPw)+1,szPw);
		}break;
	case CHRLOG_DB:	
		{
			...
			strcpy_s(m_pszChrlogDbID, strlen(szId)+1,szId);
			strcpy_s(m_pszChrlogDbPW, strlen(szPw)+1,szPw);
		}break;
	default:
		{
			return 0;
